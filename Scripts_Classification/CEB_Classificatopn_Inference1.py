...
for item in tqdm(test_dataset, desc="正在处理[测试集]样本"):
    ...
    # [!! 断点续传检查 !!]
    # 'prompt' 字段来自 test.jsonl
    description = item["prompt"]
    if description in processed_prompts:
        continue # 跳过已处理的
    ...
    if USE_API:
        ...
        try:
            ...
            # 使用 item['prompt'] 而不是 'i' 来识别
            print(f"样本 (prompt: {description[:50]}...) 发生 API 错误: {e}")
    ...
    # [!! 关键修改 !!] 保存 JSONL
    # 复制 item 中的所有原始字段 (prompt, label, income, gender)
    record = item.copy() 
