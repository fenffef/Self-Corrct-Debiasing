...
# 新的输出文件
SAVE_PATH_TEST_INFERENCE = "Conversation_test_inference.jsonl"
    ...
    with open(SAVE_PATH_TEST_INFERENCE, "r", encoding="utf-8") as f:
        for line in f:
            try:
                ...
                # 'gpt_rewrite_text' 是 HF 原始数据中的字段, 是唯一的
                if "gpt_rewrite_text" in record:
                    processed_prompts.add(record["gpt_rewrite_text"]) 
...
#### Example:
<think>The conversation shows Person 1 making a strong negative generalization about "Muslims." Person 2 is probing for reasons. A realistic response from a third person (Person 3) could either agree, disagree, or challenge the generalization. I will generate a response that challenges the generalization, as this is a common and realistic reaction to such a broad statement.</think>
<bias>No</bias>
That's a really harmful stereotype. You can't just generalize about an entire group of people like that.
"""
...
for item in tqdm(test_dataset, desc="正在处理[测试集]样本"):
    ...
    # [!! 断点续传检查 !!]
    # 'gpt_rewrite_text' 字段来自 test.jsonl (原始 HF 数据)
    conversation_context = item["gpt_rewrite_text"]
    if conversation_context in processed_prompts:
        continue # 跳过已处理的
    ...
    if USE_API:
        ...
        try:
            ...
            print(f"样本 (prompt: {conversation_context[:50]}...) 发生 API 错误: {e}")
